...
const { aegis, aegisSingle, getNukes } = require('../commands/implementations/aegis');
...
const { addban, addmute, addphrase } = require('../commands/implementations/banphrase');
const { unbanphrase, removephrase } = require('../commands/implementations/unbanphrase');
...
function registerCommandsFromFiles(commandRegistry, chatConnectedTo, config) {
  ...
  commandRegistry.registerCommand('!aegissingle', aegisSingle, ['!unnuke', '!an', '!as']);
  commandRegistry.registerCommand('!getnukes', getNukes, ['!gn']);
  ...
  if (chatConnectedTo === 'dgg') {
    commandRegistry.registerCommand('!addphrase', addphrase, ['!addban', '!addmute']);
    commandRegistry.registerCommand('!removephrase', removephrase, ['!removeban', '!removemute', '!deletephrase', '!deleteban', '!deletemute', '!dmute', '!dban', '!dphrase']);
  ...
  if (chatConnectedTo === 'twitch') {
    ...
    commandRegistry.registerCommand('!removephrase', unbanphrase, ['!removeban', '!removemute', '!deletephrase', '!deleteban', '!deletemute', '!dmute', '!dban', '!dphrase']);
  }
}
