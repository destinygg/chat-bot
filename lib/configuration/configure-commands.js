...
const { duel } = require('../commands/implementations/duel');
...
const version = require('../commands/implementations/version');
...
function registerCommandsFromFiles(commandRegistry, chatConnectedTo, config) {
  ...
  const mutelinksTrustedFlairs = (config.mutelinks && config.mutelinks.trustedFlairs) || [];
  commandRegistry.registerCommand('!mutelinks', mutelinks(60, mutelinksTrustedFlairs), [
    '!mutelink',
    '!linkmute',
    '!linksmute',
  ]);
  commandRegistry.registerCommand('!breakingnews', breakingNews, ['!breaking', '!bn']);
  commandRegistry.registerCommand('!version', version, ['!v']);
  ...
  if (chatConnectedTo === 'dgg') {
    ...
    commandRegistry.registerCommand('!duel', duel);
  }
