...
class CommandRouter {
   ...
   * @type {Object}
   * @property {import("../chat-utils/parse-commands-from-chat").ParsedCommand} parsedCommand
   * @property {import("../chat-utils/parse-commands-from-chat").ParsedMessage} parsedMessage
   ...
   * @returns {Promise<CommandResult|import("../commands/command-output")>}
   */
  runCommand(command, input, parsedMessage, isWhisper) {
    return new Promise((accept, reject) => {
      if (command.isPromiseOutput) {
        const workResult = command.work(input, this.services, parsedMessage);
        return /** @type {Promise<import("../commands/command-output")>} */ (workResult)
          .then((outputObject) => {
      ...
      try {
        ...
        return accept({
          user: parsedMessage.user,
          output: /** @type {import("../commands/command-output")} */ (staticOutputObject).output,
          isMultiLine: command.multiLineCommand,
          isWhisper,
        });
      } catch (e) {
