...
class ChatServiceRouter {
  constructor(chatToConnectTo, bot, messageRouter, commandRouter, logger,
    punishmentStream, messageSchedulerStream, fakeScheduler, messageRelay) {
    ...
    this.bot.on('command', (commandObject) => {
      this.commandRouter.routeIncomingCommandMessage(commandObject)
        .then((outputObject) => {
          ...
          if (this.chatToConnectTo === 'dgg' && outputObject.isMultiLine && outputObject.isWhisper === false) {
            this.bot.sendMultiLine(outputObject.output);
          } else if (this.chatToConnectTo === 'twitch' && outputObject.isMultiLine && outputObject.isWhisper === false) {
