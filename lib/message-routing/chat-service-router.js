...
class ChatServiceRouter {
  ...
  create() {
    ...
    this.bot.on('err', (message) => {
      this.messageRelay.relayMessageToListeners('err', message);
    });

    this.bot.on('pollstart', (message) => {
      this.messageRelay.relayMessageToListeners('pollstart', message);
    });
    ...
    this.punishmentStream.on('data', (punishmentObject) => {
      ...
      if (!_.isEmpty(punishmentObject.reason)) {
