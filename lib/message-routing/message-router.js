...
class MessageRouter {
  ...
  routeIncomingMessages(newMessage) {
    ...
    const nukeDuration = isMessageNuked(nukedPhrases, messageContent);
    if (nukeDuration !== 0) {
      this.punishmentStream.write(makeMute(user, nukeDuration, `${user} muted for ${nukeDuration} seconds for using a recently nuked phrase.`));
      return;
    }
