...
class MessageRouter {
  ...
  routeIncomingMessages(newMessage) {
    ...
    if (nukeResult.duration !== 0) {
      this.punishmentCache.addUserToAegisCache(user, nukeResult.duration * 1000);
      this.punishmentStream.write(makeMute(user, nukeResult.duration, `${user}
       muted for ${formatDuration(moment.duration(nukeResult.duration, 'seconds'))} for using a recently nuked phrase (${nukeResult.phrase}).`));
      return;
    }
