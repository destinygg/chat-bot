...
class PunishmentCache {
  constructor(config) {
    ...
    this.flairArmorMap = config.flairBasedBanDodgeChances || {};
  ...
  tombStoneCleanup() {
    ...
    _.forIn(this.singleUserAegisCache, (value, key) => {
      if (now >= value.timestamp) {
        delete this.singleUserAegisCache[key];
      }
    });
  }
  ...
  addNukeToCache(userList, nukedPhrase, duration, isMegaNuke) {
    ...
    setTimeout(() => {
      this.cleanseSingleNuke(nukedPhrase);
  ...
  getNukedUsers() {
    return _.keys(this.singleUserAegisCache);
  }
