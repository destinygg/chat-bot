...
class PunishmentReadWriteStream extends Transform {
  ...
  handleMute(punished) {
    ...
    if (punished.isBannedWord === true) {
      ...
      this.sendMute(`${punished.reason} Length: ${formatLength(muteDuration)}.`,
        muteDuration, punished.type, punished.user);
      return;
    }

    if (punished.duration === null) {
      muteDuration = this.punishmentCache.getAndAddPunishmentDuration(punished.user);
      muteReason = `Muting ${punished.user} for ${formatLength(Math.ceil(muteDuration))} due to spam detection. Reason: ${punished.reason}`;
    } else {
