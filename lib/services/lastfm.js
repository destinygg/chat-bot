  ...
  getCurrentPlayingSong() {
    return this.fetchLastScrobbledTrack(this.username, 1).then(body => {
      ...
      if (lastPlayed) {
        ...
        const date = _.get(lastPlayed, 'date', false);

        if (nowPlaying || date === false) {
          return {
            trackName,
            artistName,
            nowPlaying,
        ...
        const lastPlayedTimestamp = moment.unix(date.uts);
  ...
  getPreviousSongPlaying() {
    return this.fetchLastScrobbledTrack(this.username, 2).then(body => {
      ...
      if (lastPlayed && previouslyPlayed) {
        ...
        const playedAgo = moment.duration(previouslyPlayedTimestamp.diff(lastPlayedTimestamp));
        return {
          lastPlayedTrackName,
