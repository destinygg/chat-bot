...
class ChatCache {
  ...
  addMessageToViewerUrlMap(user, message) {
    this.services.logger.info('Checking message for url mapping', message);
    if (
      this.messageUrlSpamUser &&
      !this.services.messageMatching.mentionsUser(message, this.messageUrlSpamUser)
    ) {
      return;
    }
    this.services.messageMatching.getLinks(message).forEach((link) => {
    ...
    this.services.logger.info('User Url Map', this.viewerUrlMap);
  }
