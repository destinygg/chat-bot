...
class ChatCache {
  ...
  tombStoneCleanup() {
    ...
    _.forIn(this.viewerUrlMap, (urls, user) => {
      const filteredOldUrls = urls.filter(
        (unixurl) => now - parseInt(unixurl.split('%')[0], 10) < this.messageUrlSpamSeconds,
      );
  ...
  addMessageToViewerUrlMap(user, message) {
    ...
    this.messageMatching.getLinks(message).forEach((link) => {
      ...
      this.viewerUrlMap[user].push(`${moment().unix()}%${link.hostname + link.pathname}`);
    });
  }
  ...
  getViewerUrlList(user) {
    ...
    const filteredOldUrls = this.viewerUrlMap[user]
      .filter((unixurl) => now - parseInt(unixurl.split('%')[0], 10) < this.messageUrlSpamSeconds)
      .map((unixurl) => unixurl.split('%')[1]);
