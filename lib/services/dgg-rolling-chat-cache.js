...
class ChatCache {
  constructor(config, services) {
    ...
    this.viewerMap = {};
    this.viewerUrlMap = {};
    // Array to store recently posted URLs by any user, used for global URL spam detection
    this.recentUrls = [];
    this.rateLimitMap = {};
  ...
  tombStoneCleanup() {
    ...
    // Clean up old URLs from recentUrls
    this.recentUrls = this.recentUrls.filter((link) => now - link.timestamp < this.urlTtlSeconds);
  ...
  addMessageToViewerUrlMap(user, message) {
    ...
    // Track all URLs regardless of mentions
    this.messageMatching.getLinks(message).forEach((link) => {
      const url = this.normalizeUrl(link);
  ...
  getRecentUrls() {
    ...
    return this.recentUrls
      .filter((link) => now - link.timestamp < this.urlTtlSeconds)
      .map((link) => link.url);
  }
