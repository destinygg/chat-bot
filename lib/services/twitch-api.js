...
class TwitchApi {
  ...
  getChannelStatus() {
    return axios.get('https://api.twitch.tv/kraken/streams/destiny', {
      ...
      return `Viewers: ${viewers}. Stream live as of ${formattedDuration}`;
    });
  }
  ...
  getPastBroadcastInformation() {
    ...
    return this.sql.getTwitchTimestamp(this.timeStampKey)
      .then((timestamp) => {
        ...
        return axios.get('https://api.twitch.tv/kraken/channels/destiny/videos?limit=1&broadcast_type=archive', {
          headers:
            {
              'Client-ID': this.clientId,
            },
