...
class TwitchApi {
  ...
  getChannelStatus() {
    return axios.get('https://api.twitch.tv/kraken/streams/18074328', {
      headers:
        {
      ...
      const now = moment();
      const formattedDuration = formatDuration(moment.duration(now.diff(moment(date, 'YYYY-MM-DDTHH:mm:ssZ'))));
  ...
  getPastBroadcastInformation() {
    ...
    return this.sql.getTwitchTimestamp(this.timeStampKey)
      .then((timestamp) => {
        ...
        const formattedDuration = formatDuration(moment.duration(now.diff(moment.tz(timestamp, 'X', 'UTC'))));
        context.timestamp = `Stream was last online ${formattedDuration} ago.`;
        return axios.get('https://api.twitch.tv/kraken/channels/18074328/videos?limit=1&broadcast_type=archive', {
          headers:
            {
              Accept: 'application/vnd.twitchtv.v5+json',
              'Client-ID': this.clientId,
      ...
      .then((data) => {
        ...
        return `${context.timestamp} Time Streamed: ${formatDuration(moment.duration(length, 'seconds'))}`;
      });
  }
}
