...
class TwitchChatListener extends EventEmitter {
  ...
  connect() {
    ...
    // this.client.on('PRIVMSG', this.parseMessages.bind(this)(true));
  ...
  sendWhisper(user, message) {
    this.client.whisper(user, message).then().catch((err) => {
  ...
  parseMessages(channel, userState, message, self) {
    ...
    if (userState['message-type'] === 'chat' || userState['message-type'] === 'whisper') {
