...
function handleGulagStartOfMatch(runner) {
  if (runner.prisoners.length !== 0) {
    if (runner.prisoners.length === 1) {
      ...
      runner.services.messageRelay.sendOutputMessage(
        'The audience can use !stone <user> to cancel out to cancel out a submitted answer',
      );
      setTimeout(() => {
...
class GulagRunner extends EventEmitter {
  constructor(services, config) {
    ...
    this.matchDurationMilliseconds = config.matchDurationMilliseconds || 10000;
    this.muteDurationSeconds = config.muteDurationSeconds || 3600;
