...
class YouTube {
  ...
  getChannelsUploadedPlaylistId(user) {
    return this.youtube.channels.list({
      ...
      .then(response => _.find(response.data.items, ['kind', 'youtube#channel']).contentDetails.relatedPlaylists.uploads);
  }
}
