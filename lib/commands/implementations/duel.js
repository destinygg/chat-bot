...
function duel(defaultMuteDuration) {
  ...
  return (input, services, rawMessage) => {
    ...
    if (listener === false) {
      return new CommandOutput(null, 'Something went wrong starting the duel. Try again.');
    }
    ...
    listener.on('msg', (data) => {
      ...
      services.punishmentStream.write(
        makeMute(
          loser,
          muteDuration,
          `${loser} lost a duel to ${winner}, started by ${rawMessage.user}.`,
        ),
      );
    });
    ...
    return new CommandOutput(
      null,
      `DUEL! ${user1} vs ${user2} -- First to type "${displayPhrase}" wins. Loser gets muted for ${muteString}. You have 30 seconds.`,
    );
  };
}
