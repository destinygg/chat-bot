...
function nuke(input, services, rawMesasge) {
  ...
  const nukeWord = _.get(matched, 2).toLowerCase();
  const duration = _.get(matched, 1, '');
  let parsedDuration = 600;
  if (duration !== '') {
    parsedDuration = parseDurationToSeconds(duration);
    if (parsedDuration === null) {
      return new CommandOutput(null, null, 'Could not parse the duration. Usage: "!nuke {amount}{m,h,d,w} {some nuke phrase} " !nuke 1d GET GNOMED');
    }
  }
  ...
  return new CommandOutput(null, null, `Dropping the NUKE on ${userList.length} victims`);
}
