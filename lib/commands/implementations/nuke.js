...
function nuke(isMegaNuke, isGulagRoundup) {
  return (input, services, rawMessage) => {
    ...
    userList.forEach(user => {
      ...
      if (isGulagRoundup) {
        services.gulagRunner.addUserToGulag(user);
      }
    });
    return new CommandOutput(
      null,
      isGulagRoundup
        ? `Sending ${userList.length} prisoners to the gulag`
        : `Dropping the NUKE on ${userList.length} victims`,
    );
  };
}
