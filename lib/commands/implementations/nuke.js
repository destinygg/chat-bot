...
function nuke(input, services, rawMesasge) {
  ...
  services.punishmentCache.addNukeToCache(userList, wordToMute, parsedDuration);
  userList.forEach(user => {
    services.punishmentStream.write(makeMute(user, parsedDuration, false));
  });
  return new CommandOutput(null, `Dropping the NUKE on ${userList.length} victims`);
}

module.exports = new Command(nuke, false, true, /(\d+[HMDSWwhmds])?\s?(?:\/(.*)\/)?(.*)/, false);
