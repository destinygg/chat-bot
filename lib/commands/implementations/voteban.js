...
function voteBan(ipBan, defaultBanTime) {
  return function ban(input, services, rawMessage) {
    ...
    return new CommandOutput(null, `/vote Should we ban ${userToPunish} ${muteString === 'PERMANENTLY' ? muteString : `for ${muteString}`} ${parsedReason ? ` for ${parsedReason}` : ''}? yes or no 30s`);
  };
}
