...
function updateDuo(input, services) {
  ...
  const duoText = _.get(matched, 1).toLowerCase();

  return services.sql.updateDuo(duoText)
    .then(() => new CommandOutput(null, 'Duo text updated')).catch(err => new CommandOutput(err, "Oops. Something didn't work. Check the logs."));
}

function getDuo(input, services) {
  return services.sql.getDuoText()
