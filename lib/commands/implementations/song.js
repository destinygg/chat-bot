...
function getCurrentPlayingSongForBroadcaster(input, services) {
  return services.lastfm.getCurrentPlayingSong()
    .then((song) => {
      ...
      if (song.nowPlaying) {
        return new CommandOutput(null, `"${song.trackName} - ${song.artistName}" ${profilePageUrl}`);
      }
      return new CommandOutput(null, `Not playing any songs. Last played "${song.trackName} - ${song.artistName}" ${song.playedAgo.humanize(true)} ${profilePageUrl}`);
    })
