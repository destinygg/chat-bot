...
function addCommand(input, services) {
  ...
  if (services.commandRegistry.findCommand(newCommand)) {
    return Promise.resolve(new CommandOutput(null, `Command '${newCommand}' already exists.`));
  }
  return services.sql
    .addCommand(newCommand, commandText)
    .then(() => {
      services.commandRegistry.registerCommand(
        newCommand,
        new Command(staticOutput(commandText), false, null, null),
      );
      return new CommandOutput(null, `Added new command: ${newCommand}`);
    })
