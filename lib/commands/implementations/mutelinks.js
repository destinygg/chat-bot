...
function mutelinksFactory(defaultPunishmentDuration, trustedFlairs = []) {
  ...
  const isTrustedUser = (data) => {
    if (!trustedFlairs.length) return false;
    const roles = data.roles || [];
    return roles.some((role) => trustedFlairs.includes(role));
  };
    ...
    if (state === 'on') {
      ...
      listener.on('msg', (data) => {
        ...
        const hasRepeatedLink = links.some((link) => {
          ...
          services.logger.info('Testing URL in mutelinks repeat mode', { url: normalizedUrl });
...
module.exports = {
  mutelinks: (defaultPunishmentDuration, trustedFlairs) =>
    new Command(
      mutelinksFactory(defaultPunishmentDuration, trustedFlairs),
      false,
      true,
