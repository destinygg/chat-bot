...
function mutelinks(defaultPunishmentDuration) {
  ...
  return (input, services, rawMessage) => {
    const matched = /(on|off|all)\W(\d+[HMDSWwhmds])?/.exec(input);
    ...
    listener.on('message', data => {
      ...
      if (state !== 'all' && !message.includes(mentionUser)) return;
...
module.exports = {
  mutelinks: new Command(mutelinks(600), false, true, /(on|off|all)\W(\d+[HMDSWwhmds])?/, false),
};
