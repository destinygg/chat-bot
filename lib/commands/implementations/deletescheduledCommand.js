...
function deleteScheduledCommand(input, services) {
  ...
  if (commandKey === '') {
    return new CommandOutput(
      null,
      `Command '${commandKey}' not parsed. Must be in format !commandtodelete`,
    );
  }
  if (services.commandRegistry.findCommand(commandKey) === false) {
    return Promise.resolve(
      new CommandOutput(null, 'Command was not scheduled, so nothing was removed.'),
    );
  }
  return services.sql
    ...
    .catch((err) => {
      if (err.message === 'Command not found') {
        return new CommandOutput(
          err,
          `Command ${commandKey} was not scheduled, and nothing was removed`,
        );
      }
