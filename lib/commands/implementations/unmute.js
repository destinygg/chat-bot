...
function unmute(input, services) {
  ...
  if (_.isEmpty(userToUnmute)) {
    return new CommandOutput(
      ...
      `Something did not work. ${userToUnmute} not unmuted.`,
    );
  }
  services.punishmentStream.write(makeUnmute(userToUnmute));
  return new CommandOutput(null, null, null);
}

module.exports = new Command(unmute, false, true, /(\w+)/, null);
