...
async function live(input, services) {
  try {
    ...
    let output = '';
    if (twitchStatus.isLive && youtubeStatus.isLive) {
      output = `Viewers: ${totalViewers}, TTV: ${twitchViewers}, YT: ${youtubeViewers}. Stream live as of ${formattedDuration} ago`;
    } else {
      output = `Viewers: ${totalViewers}. Stream live as of ${formattedDuration} ago`;
    }
