...
function getPreviousPlayingSongForBroadcaster(input, services) {
  return services.lastfm.getPreviousSongPlaying()
    .then((song) => {
      ...
      return new CommandOutput(null, `"${song.previouslyPlayedTrackName} - ${song.previouslyPlayedArtistName}" played before "${song.lastPlayedTrackName} - ${song.lastPlayedArtistName}" ${song.playedAgo.humanize(true)} ${profilePageUrl}`);
    })
