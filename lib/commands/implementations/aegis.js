...
function aegis(input, services) {
  ...
  if (usersToUnmute.length === 0 && services.punishmentCache.getLastNukeList().length === 0) {
  ...
  return new CommandOutput(null, `Unmuting ${usersToUnmute.length} users AngelThump`);
}
