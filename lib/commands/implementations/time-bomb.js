...
function timeBomb(input, services, rawMessage) {
  const matched = /((?:\d+[HMDSWwhmds])\s+|(?:perm)\s+)?((?:\d+[HMDSWwhmds])\s+)?\s?(\w+(?:\s*,\s*\w+)*)(?:\s(.*))?/.exec(input);
  ...
  if (parsedInput === false || users === 'null') {
...
function defuse(input, services) {
  ...
  if (!users || users[0] === 'null') {
    ...
    const length = Object.keys(bombs).length;
    const plural = length > 1? 's' : '';
    return new CommandOutput(null, `Defusing ${length} bomb${plural}. AngelThump`)
  }
...
module.exports = {
  timeBomb: new Command(
    ...
    false,
    true,
    /((?:\d+[HMDSWwhmds])\s+|(?:perm)\s+)?((?:\d+[HMDSWwhmds])\s+)?\s?(\w+(?:\s*,\s*\w+)*)(?:\s(.*))?/,
    null,
  ),
  defuse: new Command(
    defuse,
