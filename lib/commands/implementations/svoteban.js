...
function svoteBan(ipBan, defaultBanTime) {
  return function ban(input, services, rawMessage) {
    if (voteBanStarted) {
      return new CommandOutput(
        null,
        'P2W Vote ban in progress. Please wait for the current vote to finish.',
      );
    }
    ...
    if (parsedInput === false) {
      return new CommandOutput(
        null,
        'Could not parse the duration. Usage: "!voteban {amount}{m,h,d,w}OR{perm} {user}" !svoteban 1d Destiny',
      );
    }
    ...
    listener.on('message', data => {
      ...
      if (message.trim() === '1' || message.trim() === '2') {
        const votes = _.max(
          Object.keys(translateMap).map(k => {
            const idx = data.roles.indexOf(k);
            if (idx === -1) return 1;
            return translateMap[k];
          }),
        );
    ...
    setTimeout(() => {
      ...
      services.punishmentStream.write(
        makeBan(
          ...
          ipBan,
          isPermanent,
          `${userToPunish} banned through bot by a P2W VOTE BAN started by ${rawMessage.user}. ${
    ...
    return new CommandOutput(
      null,
      `/svote P2W Vote: Should we ban ${userToPunish} ${
