...
function addScheduledCommand(input, services) {
  ...
  if (commandKey === '') {
    return new CommandOutput(null, `Command '${commandKey}' to schedule was not parsed. Must be in the format !CommandToSchedule`);
  }
  if (services.commandRegistry.findCommand(commandKey) === false) {
    return Promise.resolve(new CommandOutput(null, `Command '${commandKey}' not scheduled. Command must be registered first with !addcommand`));
  }
  return services.sql.addScheduledCommand(commandKey)
    .then(() => {
